...
[![Node.js CI](https://github.com/danielmartincraig/pick-things-up/actions/workflows/node.js.yml/badge.svg)](https://github.com/danielmartincraig/pick-things-up/actions/workflows/node.js.yml)
...
* [`src/pick_things_up/`](src/pick_things_up/): SPA source files (ClojureScript,
[re-frame](https://github.com/Day8/re-frame))
  - [`core.cljs`](src/pick_things_up/core.cljs): contains the SPA entry point, `init`
* [`test/pick_things_up/`](test/pick_things_up/): test files (ClojureScript,
...
(headless test environment)
    * For Chromium, set the `CHROME_BIN` environment variable in your shell to the command that
    launches Chromium. For example, in Ubuntu, add the following line to your `.bashrc`:
       ...
       ```
5. Clone this repo and open a terminal in the `pick-things-up` project root directory
...
The `debug?` variable in [`config.cljs`](src/cljs/pick_things_up/config.cljs) defaults to `true` in
[`dev`](#running-the-app) builds, and `false` in [`prod`](#production) builds.

Use `debug?` for logging or other tasks that should run only on `dev` builds:

```clj
(ns pick-things-up.example
  (:require [pick-things-up.config :as config])
